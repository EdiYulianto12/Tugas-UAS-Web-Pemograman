...
if (isset($_POST['bSimpan'])) {
  ...
  $namaranggota=$_POST['namaanggota'];
  $alamat=$_POST['alamat'];
  $tanggaldaftar=$_POST['tanggaldaftar'];
  $tanggalkadaluarsa=$_POST['tanggalkadaluarsa'];
  $koneksi=new mysqli("localhost","root","","perpustakaan");
  $sql="UPDATE `pustakawan` SET `nomoranggota` = '$nomoranggota', `namaanggota` = '$namaanggota',  `alamat` = '$alamat',  `tanggaldaftar` = '$tanggaldaftar',  `tanggalkadaluarsa` = '$tanggalkadaluarsa' WHERE `anggota`.`nomoranggota` = '$nomoranggota';";
  $q=$koneksi->query($sql);
  if ($q) {
